...
export default function BlogPost({ title, date, excerpt, category, slug, readTime }: BlogPostProps) {
  ...
  return (
    <article className="border border-retro-border p-6 hover:glow-retro transition-all duration-300 bg-retro-bg/50 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <time className="text-retro-text text-xs font-mono">{formattedDate}</time>
        </div>
      </div>
      <h3 className="text-retro-text text-xl md:text-2xl mb-4 text-shadow-retro">
        {title}
      </h3>
      <p className="text-retro-text text-base md:text-lg leading-relaxed mb-4">
        {excerpt}
      </p>
      <Link 
        href={`/writings/${slug}`}
