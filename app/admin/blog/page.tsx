    ...
    <main className="min-h-screen relative p-6 md:p-8">
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <form onSubmit={handleSubmit} className="border border-retro-border p-6 md:p-8 bg-retro-bg/50 backdrop-blur-sm space-y-6">
          ...
          >
            {isSubmitting ? 'SUBMITTING...' : 'ADD WRITING'}
          </button>

          {submitStatus === 'success' && (
            <div className="text-retro-text text-sm text-center border border-retro-border px-4 py-2">
              WRITING ADDED SUCCESSFULLY! (Check console for data)
            </div>
          )}
          {submitStatus === 'error' && (
            <div className="text-retro-muted text-sm text-center border border-retro-border px-4 py-2">
              ERROR ADDING WRITING. PLEASE TRY AGAIN.
            </div>
          )}
        </form>

        <div className="mt-8 p-4 border border-retro-accent text-retro-accent text-sm">
          <p className="mb-2">&gt; NOTE:</p>
          <p>This is a demo admin page. In production, you would:</p>
          <ul className="list-disc list-inside mt-2 space-y-1 ml-4">
            <li>Save writings to a database or CMS</li>
            <li>Add authentication/authorization</li>
            <li>Implement image uploads</li>
            <li>Add editing and deletion functionality</li>
          </ul>
        </div>
      </div>
    </main>
